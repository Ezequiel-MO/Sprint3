  ...
  if (cart.some((item) => selectedItem.name === item.name)) {
    ...
    if (cart[index].name === "cooking oil") {
      if (cart[index].quantity >= 3) {
        cart[index].subtotalWithDiscount =
          cart[index].quantity * (cart[index].price - 0.5);
      }
...
const deleteSubtotalWithDiscountProperty = (selectedItem) =>
  delete selectedItem.subtotalWithDiscount;

const updateSubtotalWithDiscount = (
  selectedItem,
  isCookingOil = false,
  isInstantCupcakeMixture = false
) => {
  if (isCookingOil) {
...
const checkIfDiscountsApplyAndUpdate = (selectedItem) => {
  ...
  const isInstantCupcakeMixture =
    selectedItem.name === "Instant cupcake mixture";
  ...
  if (isCookingOil) {
    if (LessThan3Units) {
      ...
      updateSubtotalWithDiscount(selectedItem, isCookingOil);
    }
  } else {
    if (isInstantCupcakeMixture) {
      if (LessThan10Units) {
        ...
        updateSubtotalWithDiscount(selectedItem, isInstantCupcakeMixture);
      }
    }
  }
};
...
const findPositionInCartItemToRemove = (selectedItem) =>
  cart.findIndex((item) => item.name === selectedItem.name);

const removeSelectedItemfromCart = (position) => {
  cart.splice(position, 1);
};

const isSelectedItemInCart = (selectedItem) =>
  cart.some((item) => selectedItem.name === item.name);
